...
from .utils import mask2one_hot
from .seg_model import unetr
import torch.nn.functional as F

class TrainerP95(Trainer):
    ...
    def train(self):
        ...
        for i in range(self.total_iter_nums):
            ...
            # target = mask2one_hot(target[0], pred)
            # import pdb
            # pdb.set_trace()
    ...
    def test(self):
        ...
        if self.output_dir is None:
            ...
            # import pdb
            # pdb.set_trace()
            tifffile.imsave(os.path.join(self.output_dir, f'{self.cfg.INFERENCE.OUTPUT_NAME}.tif'), result[0].astype(np.uint8))
    ...
    def save_checkpoint(self, iteration: int, is_best: bool = False):
        ...
        if self.is_main_process:
            ...
            if self.cfg.MODEL.ARCHITECTURE == 'sam_vit_b':
    ...
    def update_checkpoint(self, checkpoint: Optional[str] = None):
        ...
        if 'state_dict' in checkpoint.keys():
            ...
            # import pdb
            # pdb.set_trace()
            model_dict = self.model.state_dict()  # nn.DataParallel
            ...
            # import pdb
            # pdb.set_trace()
            self.model.load_state_dict(model_dict)  # nn.DataParallel
